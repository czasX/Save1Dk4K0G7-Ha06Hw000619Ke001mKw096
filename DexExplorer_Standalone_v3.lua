...
pcall(function()
    local old = ROOT_GUI:FindFirstChild("DexExplorerV3")
...
-- Source viewer. Roblox clients normally cannot read Script.Source;
-- if Source is inaccessible, the viewer explains that limitation instead
-- of pretending the source was available.
local sourceFrame = mk("Frame", {
    Name="SourceViewer",
    Size=UDim2.new(.78,0,.78,0),
    Position=UDim2.new(.11,0,.11,0),
    BackgroundColor3=CFG.Bg2,
    BorderSizePixel=0,
    Visible=false,
    ZIndex=50,
}, main)
round(sourceFrame,10)
outline(sourceFrame,CFG.Accent,1)

local sourceHeader = mk("Frame", {
    Size=UDim2.new(1,0,0,38),
    BackgroundColor3=CFG.Bg3,
    BorderSizePixel=0,
    ZIndex=51,
}, sourceFrame)
round(sourceHeader,10)

local sourceTitle = mk("TextLabel", {
...
local sourceClose = mk("TextButton", {
...
sourceClose.MouseButton1Click:Connect(function()
    sourceFrame.Visible=false
end)

local function showSourceViewer(obj)
    if not obj then return end

    sourceTitle.Text="SOURCE • "..obj.Name

    if not obj:IsA("LuaSourceContainer") then
        sourceBox.Text="This instance does not contain Luau source."
    ...
    if ok and type(source)=="string" then
        ...
        sourceBox.Text=
            "Source is not readable from this runtime.\n\n"..
            "Roblox normally does not replicate Script.Source / LocalScript.Source / ModuleScript.Source to a client.\n"..
            "Use Studio or a server-side source system for source inspection in a game you control."
    end
...
local SEARCH_CACHE = {}
local SEARCH_CACHE_VALID = false
local SEARCH_BUILD_TOKEN = 0

local function invalidateSearchCache()
    SEARCH_CACHE_VALID = false
    SEARCH_BUILD_TOKEN += 1
end

local function buildSearchCache()
    if SEARCH_CACHE_VALID then
        return
    end

    SEARCH_BUILD_TOKEN += 1
    local token = SEARCH_BUILD_TOKEN
    table.clear(SEARCH_CACHE)
    ...
    local processed = 0
    while #stack > 0 do
        if token ~= SEARCH_BUILD_TOKEN then
            return
        end

        local obj = table.remove(stack)
        SEARCH_CACHE[obj] = getSearchBlob(obj)
...
local function matches(obj, query)
    ...
    local blob = SEARCH_CACHE[obj]
    if not blob then
        blob = getSearchBlob(obj)
        SEARCH_CACHE[obj] = blob
    end
...
local function showInspector(obj)
    ...
    action("VIEW SOURCE",0,function() showSourceViewer(obj) end,100)
    action("COPY PATH",106,function() safeCopy(getPath(obj)) end,88)
    action("COPY NAME",200,function() safeCopy(obj.Name) end,88)
    action("COPY CLASS",294,function() safeCopy(obj.ClassName) end,88)
end
...
function refreshTree()
    ...
    local function scan(parent,depth)
        ...
        for _,child in ipairs(children) do
            ...
            if not matchSet or matchSet[child] then
                count+=1
                if renderedCount < MAX_RENDERED then
                    order+=1
                    renderedCount+=1
    ...
    if renderedCount < count then
        countLabel.Text=tostring(renderedCount).."/"..tostring(count).." visible"
    end
    if query~="" then
...
search:GetPropertyChangedSignal("Text"):Connect(function()
    searchDebounce += 1
    local id = searchDebounce

    task.delay(.12,function()
        if id ~= searchDebounce then return end
        if not gui.Parent then return end

        local ok,err = pcall(refreshTree)
        if not ok then
            setStatus("Search error: "..tostring(err),CFG.Bad)
        end
    end)
end)
...
local function watchHierarchy(obj)
